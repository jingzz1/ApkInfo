 ...
 */
public class SlidingShutManager {

    /**
     * 起始触点的X轴范围
     * 大于这个范围则不会触发右滑关闭
     */
    private int SLIDE_SCREEN_WIDTH_SCALE = 1;
     ...
     * 滑动事件处理类协助类
     */
    private SlidingShutScrollHelper slidingShutHelper;
     ...
     * @param activity
     */
    public SlidingShutManager(Activity activity) {
        ...
        //初始化SlidingShutScrollHelper，并且绑定rootView
        slidingShutHelper = new SlidingShutScrollHelper();
        slidingShutHelper.setRootView(rootView);
        DisplayMetrics displayMetrics = getScreen(activity);
    ...
    public void setRootView(View rootView) {
        ...
        //SlidingShutScrollHelper绑定rootView
        if(slidingShutHelper != null)
            slidingShutHelper.setRootView(rootView);
    }
    ...
    /**
     * 右滑时不拦截屏幕的比例，默认是1（100%），即在屏幕任何地方滑动都能响应
     * @param SlideScreenWidthScale
     */
    public void setSlideScreenWidthScale(int SlideScreenWidthScale) {
        this.SLIDE_SCREEN_WIDTH_SCALE = SLIDE_SCREEN_WIDTH_SCALE;
    }
    ...
    private boolean actionDown(MotionEvent motionEvent) {
        ...
        //计算y轴
        startSlidingY = (int) motionEvent.getY();
        //计算触点位置的所有子控件，方便在滑动时做处理
        slidingShutHelper.onDown(startSlidingX, startSlidingY);
        // 添加进度条
        if (mProgress == null) {
            // 初始化进度条
            initProgress();
    ...
    private boolean actionMove(MotionEvent motionEvent) {
        ...
        if(Math.abs(startSlidingY - nowY) > Math.abs(startSlidingX-nowX))
            return false;

        if (false == slideOpen ||
                rootView == null ||
                startSlidingX > mScreenWidth / SLIDE_SCREEN_WIDTH_SCALE
                ||!slidingShutHelper.canScroll((int)(startSlidingX-nowX))) {
            return false;
        }
